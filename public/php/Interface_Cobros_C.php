...
if (!isset($_SESSION['idUsuario']) || ($_SESSION['rol'] ?? 0) != 4) {
...
$sql = "
SELECT DISTINCT vw.*
FROM vw_abonos_adeudos vw
INNER JOIN Visitas_Cobranza vc
    ON vc.idInquilino = vw.idInquilino
WHERE vc.idUsuario = ?
AND vc.Estatus IN ('Pendiente', 'En atencion')
ORDER BY vw.idAdeudo DESC
";

$stmtVisitas = $conn->prepare($sql);

if (!$stmtVisitas) {
    die("Error prepare: " . $conn->error);
}

$stmtVisitas->bind_param("i", $idUsuario);
